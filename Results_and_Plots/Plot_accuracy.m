...
function [t_uniform, S_exact, I_exact, S_apprx, I_apprx] = extract_pop_stats(k_pop)
% This function extracts (S,I) populations two algorithms:
% 1). novel exact algorithm;
% 2). existing approximate time-driven algorithm;
% with respect to a 'golden standard' algorithm.
% dimensions of S_exact (sim. I_ex, S_apprx, I_apprx): (rep, timesteps, delta)
...
%approx_iterTime_mean = zeros(length(setup), length(delta_t));
    ...
    S_exact = stocha_info_S_effiMulti_deltat(:, 1:end, :);
    I_exact = stocha_info_I_effiMulti_deltat(:, 1:end, :);
    S_apprx = stocha_info_S_approx_deltat(:, 1:end, :); 
    I_apprx = stocha_info_I_approx_deltat(:, 1:end, :);
...
function plot_SI_timeseries(deltat_k, k_pop)
    [t_uniform, S_ex, I_ex, S_apprx, I_apprx] = extract_pop_stats(k_pop);
    t = t_uniform(:);
    ...
    median_S_ex = median(S_ex,1);
    median_I_ex = median(I_ex,1);
    median_S_apprx = median(S_apprx,1);
    median_I_apprx = median(I_apprx,1);  
    median_S_GS = median(GS_S,1);
    median_I_GS = median(GS_I,1);

    % 25th–75th percentile bands (over reps)
    q_S_ex     = prctile(S_ex,     [25 75], 1);
    q_I_ex     = prctile(I_ex,     [25 75], 1);
    q_S_apprx  = prctile(S_apprx,  [25 75], 1);
    q_I_apprx  = prctile(I_apprx,  [25 75], 1);

    % clean up
    S_ex_med    = squeeze(median_S_ex(:,:,deltat_k));
    I_ex_med    = squeeze(median_I_ex(:,:,deltat_k));
    S_ap_med    = squeeze(median_S_apprx(:,:,deltat_k));
    I_ap_med    = squeeze(median_I_apprx(:,:,deltat_k));
    S_gs_med    = squeeze(median_S_GS);
    I_gs_med    = squeeze(median_I_GS);
    
    S_ex_q = squeeze(q_S_ex(:,:,deltat_k));
    I_ex_q = squeeze(q_I_ex(:,:,deltat_k));
    S_ap_q = squeeze(q_S_apprx(:,:,deltat_k));
...
fill([t(idx); flipud(t(idx))], ...
     [I_ap_q(1,idx)'; flipud(I_ap_q(2,idx)')], ...
     'm', 'FaceAlpha', 0.15, 'EdgeColor', 'none', 'HandleVisibility','off');


    plot(t(idx), S_ex_med(idx),'b', 'LineWidth', 2, 'DisplayName','Exact S');
    plot(t(idx), I_ex_med(idx),'r', 'LineWidth', 2, 'DisplayName','Exact I');

    plot(t(idx), S_ap_med(idx),'g', 'LineWidth', 2, 'DisplayName','Approx S');
    plot(t(idx), I_ap_med(idx),'m', 'LineWidth', 2, 'DisplayName','Approx I');

    plot(t(idx), S_gs_med(idx),'--k', 'LineWidth', 2, 'DisplayName','GS S');
    plot(t(idx), I_gs_med(idx),'--k', 'LineWidth', 2, 'DisplayName','GS I');
